...
> ```
> * * * * * echo `date`+ `pwd` >> ~/testputout  
> ```

#每分钟执行一次 echo `date`+ `pwd` >> ~/testputout 命令
#将输出的内容重定向并追加到~/testputout

3.执行命令

```shell
crontab ~/testcronfile
```
...
![image-20211116220433078](crontab.assets/image-20211116220433078.png)
...
20 0-23/2 * * * echo "haha"     意思是每月每天的午夜 0 点 20 分, 2 点 20 分, 4 点 20 分....执行 echo "haha"
